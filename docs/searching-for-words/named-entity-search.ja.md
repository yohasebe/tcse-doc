...
v13.2.0 以降、固有表現型は単一トークンだけでなく**名詞チャンク**の条件としても使えます。`_{%PERSON}` は主要部が PERSON エンティティに属するチャンクにマッチし、型どうしの組み合わせや他の条件との混在も可能です。

| クエリ | マッチ対象 |
| :--- | :--- |
| `_{%PERSON}` | 人名を主要部とする名詞チャンク |
| <code>_{%PERSON&#124;%ORG}</code> | 人名または組織名を主要部とするチャンク |
| <code>_{pron&#124;%PERSON}</code> | 人である指示対象（代名詞または人名）のチャンク |
| `_{-%PERSON}` | 補集合：人名を主要部としないすべてのチャンク |

チャンク条件の全体像は[高度な検索クエリ構文](advanced-search-query-syntax.ja.md)を参照してください。
