...
- v13.5.0 からは、チャンク条件で**root のレンマ**を直接指定できます。`_{[life]}` はレンマ *life* を主要部とするチャンク（*a life*・*the lives we lead*）にマッチします。他の条件との合成（`_{[life]&noun}`）や否定（`_{-[life]}`）も可能です。例えば `[live]{verb} _{[life]}` は同族目的語のペアを直接回収します
