...
| リテラル表層形 | `['SURFACE']` | `['s]` |
| 名詞チャンク | `_` | `[give] _ _` |
| 型付き名詞チャンク | `_{COND}` | `[give] _{pron} _` |
...
v13.1.0（2026年8月）で追加。`_` は任意の名詞チャンクにマッチしますが、`_{COND}` はチャンクの**主要部（root トークン）**に条件を付けられます。条件部は通常のトークンに使う `{...}` と同じミニ言語です。

| クエリ | マッチ対象 |
| :--- | :--- |
| `_{pron}` | 代名詞を主要部とする名詞チャンク（*me*, *them* など） |
| `_{noun\|propn}` | 普通名詞または固有名詞が主要部のチャンク |
| `_{-pron}` | 補集合：代名詞主要部**でない**すべての名詞チャンク |
| `_{@nsubj}` | 主要部が `nsubj` 依存関係を持つチャンク |
| `_{%PERSON}` | PERSON エンティティを主要部とするチャンク（v13.2.0以降） |

- 1文字エイリアス（`_{v}`, `_{n}`, `_{a}`, `_{pr}` など）も使えます
- `\|`（OR）・`&`（AND）・`-`（否定）を組み合わせられます
- `_{X}` と `_{-X}` は素の `_` のマッチを厳密に二分割するため、補集合を使った網羅的な分類が可能です
- v13.2.0（2026年8月）から固有表現型も使えます。`_{%PERSON}` は主要部トークンが PERSON エンティティに属するチャンクにマッチし、`\|` で型を組み合わせられます（`_{%PERSON\|%ORG}`）。補集合 `_{-%PERSON}` は、主要部がエンティティでないチャンクも含みます。未知の型名はエラーになります

例：`[give\|send\|tell\|show\|offer] _{pron} _` は受領者が代名詞の二重目的語構文を検索します。`_{pron}` を `_{-pron}` に替えるとそれ以外のすべてが得られます。
