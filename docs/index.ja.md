...
- **品詞タグの短縮記法** — `{v}`（動詞）、`{n}`（名詞）、`{a}`（形容詞）、`{r}`（副詞）、`{pr}`（代名詞）等のショートカット追加
- **コロケーション分析** — Nグラムモードに MI（相互情報量）と t スコアによる統計分析を追加
- **対訳コンコーダンス** — 翻訳テキストで検索し、対応する英語セグメントを表示

### 構文パターン・データ

- **構文カテゴリフィルタ** — 句動詞・イディオム・文法・談話等のカテゴリ別に絞り込み可能に
- **新パターン追加** — 擬似分裂文、結果構文などの情報構造パターンを追加
- **データベース整理** — 重複・低頻度パターンを除去（全1,101パターン）
- **6,419 TED Talks** — v10 から1,100件以上のトークを追加

### UI・パフォーマンス

- **レスポンシブデザイン** — モバイル・印刷対応を改善した現代的なレイアウトに刷新
- **アクセシビリティ改善** — キーボード操作やスクリーンリーダーへの対応を強化
- **SQLクエリ最適化** — データベースインデックスの追加とクエリ性能の改善
- **ドキュメント移行** — GitBook から [GitHub Pages](https://yohasebe.github.io/tcse-doc/) に移行
