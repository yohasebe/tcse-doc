# Change font size of transcript text

Click on `S`, `M`, `L` buttons on the video controller or use corresponding keyboard shortcuts (`S`, `M`, `L` respectively) to change the font size of transcript text.

**Font small (normal viewing mode)**

![Font small in normal viewing mode](images/16.png){ width="600" }

**Font small (fullscreen viewing mode)**

![Font small in fullscreen viewing mode](images/17.png)

**Font medium (normal viewing mode)**

![Font medium in normal viewing mode](images/18.png){ width="600" }

**Font medium (fullscreen viewing mode)**

![Font medium in fullscreen viewing mode](images/19.png)
